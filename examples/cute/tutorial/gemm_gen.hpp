...
void get_gemm_gold(const size_t m, const size_t n, const size_t k,
                   const mem_layout layout_a, const mem_layout layout_b,
                   dtype_a *A, dtype_b *B, blas_type *C,
  ...
  if constexpr (std::is_same<std::remove_const_t<blas_type>, float>::value)
    cblas_sgemm(layout, transa, transb, m, n, k, 1.0, tmp_A.data(), lda,
                tmp_B.data(), ldb, 0.0, C, ldc);
  else if constexpr (std::is_same<std::remove_const_t<blas_type>,
                                  double>::value)
